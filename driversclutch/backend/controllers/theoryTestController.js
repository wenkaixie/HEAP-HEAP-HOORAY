...
const getFinalTheoryTestQuestion = async (req, res) => {
    ...
    try {
        const finalTheoryTestQuerySnapshot = await db.collection('FTT').where('sn', '==', sn).get();
  
        if (finalTheoryTestQuerySnapshot.empty) {
            return res.status(404).json({ code: 404, message: 'No document found.' });
        }
  
        const finalTheoryTestDoc = finalTheoryTestQuerySnapshot.docs[0];
        const data = finalTheoryTestDoc.data();
